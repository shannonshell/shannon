...
1. Add fish as a supported shell in shannon's Shift+Tab rotation, alongside
   bash and nushell.
...
vars, cwd, and exit code, then parses the output. Fish uses a different env
dump format than bash or nushell, so we need a new wrapper and parser.
...
Fish ships 1,055 completion definition files covering most common commands
(git, docker, cargo, npm, ssh, curl, etc.). These files use a simple
declarative format:
...
Fish's completions are actively maintained (last update: 2026-03-14). We need
...
- A `scripts/update-completions.sh` script that copies
  `vendor/fish/share/completions/*.fish` into a `completions/` directory in
  our repo (checked into git, so builds don't need fish vendored)
...
- Completions gated by `__fish_seen_subcommand_from` (specific subcommand active)
